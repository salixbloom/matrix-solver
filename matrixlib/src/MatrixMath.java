...
public class MatrixMath {
    ...
    public static void swapRow(final FlatDoubleMatrix theFlatDoubleMatrix, final int currentRow, final int targetRow) {
        ...
        } else if (currentRow > theFlatDoubleMatrix.getRows() || targetRow > theFlatDoubleMatrix.getRows()) {
        ...
        theFlatDoubleMatrix.setRow(firstRow, targetRow);
        // Swap second with first
        theFlatDoubleMatrix.setRow(secondRow, currentRow);
    }
    ...
    public static void scaleRow(final FlatDoubleMatrix theFlatDoubleMatrix, final int theRow, final double scalar) {
        ...
        theFlatDoubleMatrix.setRow(mappedData, theRow);
    }
    ...
    public static FlatDoubleMatrix ref(final FlatDoubleMatrix theFlatDoubleMatrix) {
        ...
        FlatDoubleMatrix outputFlatDoubleMatrix = new FlatDoubleMatrix(theFlatDoubleMatrix.getRows(), theFlatDoubleMatrix.getColumns());
        for (int i = 1; i < theFlatDoubleMatrix.getRows(); i++) {
            double[] rowToCopy = theFlatDoubleMatrix.getRow(i);
            outputFlatDoubleMatrix.setRow(rowToCopy, i);
        }
        // Copied matrix

        for (int i = 1; i < outputFlatDoubleMatrix.getRows(); i++) {
            if (columnNotEmpty(outputFlatDoubleMatrix, i)) {
                ...
                for (int j = i+1; j < outputFlatDoubleMatrix.getRows(); j++) {
    ...
    public static FlatDoubleMatrix rref(final FlatDoubleMatrix theFlatDoubleMatrix) {
        ...
        for (int i = outputFlatDoubleMatrix.getRows(); i > 0; i--) {
    ...
    static void main() {
        FlatDoubleMatrix theFlatDoubleMatrix = new FlatDoubleMatrix(3, 4);
        theFlatDoubleMatrix.setRow(new double[]{1, 2, 3, 1}, 1);
        theFlatDoubleMatrix.setRow(new double[]{4, 5, 6, 1}, 2);
        theFlatDoubleMatrix.setRow(new double[]{7, 8, 9, 1}, 3);
